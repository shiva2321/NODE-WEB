...
class Graph<T> implements Serializable {
    ...
    //a method that revmoves node from the graph and severes its connectiuons with other connectednodes.
    //the removal must happen at the time complexity of O(logn) or batter
    // the graph must be updated after removal and node must be removed from cache
...
class NodeNotFoundException extends RuntimeException {
    public NodeNotFoundException(String message) {
        super(message);
        message = "Node not found or marked as deleted: " + message;
